        ...
        for entry in entries {
            if let Ok(entry) = entry {
                ...
                if file_name.contains(".") { 
                    obj.folders.push(Folder {name: file_name, folders: Vec::with_capacity(0)});
                }else{
                    obj.folders.push(Folder {name: file_name.clone(), folders: Vec::with_capacity(0)});
                    get_folder_obj(format!("{}{}{}", path.clone(), "/", file_name), &mut obj.folders.last_mut().unwrap());
                }
            }
        }
    }
