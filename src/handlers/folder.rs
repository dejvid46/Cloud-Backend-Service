...
pub async fn get_folder(token: CanDownload, req: HttpRequest) -> Result<HttpResponse, ResErr> {
    ...
    let paths = fs::read_dir(path)
        .map_err(|_| ResErr::BadClientData("cant find path"))?;

    let mut res: Vec<File> = Vec::new();
    let now = SystemTime::now();

    for path in paths {
        let better_path = path.map_err(|_| ResErr::BadClientData("cant do path"))?.path();
        let metadata = fs::metadata(&better_path).map_err(|_| ResErr::BadClientData("cant get metadata"))?;
        res.push(File {
            name: better_path.clone().into_os_string().into_string().unwrap().split('/').last().unwrap().to_string(),
